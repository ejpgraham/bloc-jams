...
var findParentByClassName = function (element, targetClassName){
  ...
  while (element.parentElement){
    if (element.parentElement.className.indexOf(targetClassName) !== -1) {
      return element.parentElement;
    } else {
      element = element.parentElement;
    }
  }
  console.log("No parent with this class name found");
  return undefined;
};
...
songListContainer.addEventListener('mouseover', function(event) {
    ...
    if (targetParent.className === 'album-view-song-item' && targetParent.querySelector('.song-item-number').innerHTML !== pauseButtonTemplate ) {
    targetParent.querySelector('.song-item-number').innerHTML = playButtonTemplate;
    }
  });
